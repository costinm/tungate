...
func NewTUNFD(fd io.ReadWriteCloser, handler tungate.TUNHandler, udpNat tungate.UDPHandler) tungate.UdpWriter {
	...
	//} else {
		...
		//linkID = ep.LinkAddress()
		//stack.FindLinkEndpoint(linkIDID)
...
func (nt *GvisorTun) udpPing(NICID tcpip.NICID, stack *stack.Stack) {
	...
	go func() {
		for {
			...
			if nt.UDPHandler != nil {
				nt.UDPHandler.HandleUdp(net.IP(c.OriginalDstAddress.Addr),
					c.OriginalDstAddress.Port,
