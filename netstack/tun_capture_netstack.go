...
func NewTUNFD(fd io.ReadWriteCloser, handler tungate.TUNHandler, udpNat tungate.UDPHandler) tungate.UdpWriter {
	...
	return NewTunCapture(&linkID, handler, udpNat, false)
}
...
func DefTcpServer(nt *NetstackTun, handler tungate.TUNHandler) (tcpip.Endpoint, waiter.Queue, error) {
	...
	go func() {
		...
		for {
			...
			go func() {
				err := handler.HandleTUN(conn, nil)
				if err != nil {
					return
				}
			}()
...
func (nt *NetstackTun) DefTcp6Server() (tcpip.Endpoint, waiter.Queue, error) {
	var wq waiter.Queue
	...
	go func() {
		...
		for {
			...
			conn := gonet.NewConn(wqin, epin)
			go func() {
				err := nt.Handler.HandleTUN(conn, nil)
				if err != nil {
					return
				}
			}()
...
func setRouteTable(ipstack *stack.Stack, real bool) {
	ipstack.SetRouteTable([]tcpip.Route{
		...
		{
			Destination: tcpip.Address(net.IPv6loopback),
